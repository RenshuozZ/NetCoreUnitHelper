using System;
using System.Collections.Generic;

namespace EFCoreDBFirstGenerateModel.Models
{
    public partial class TReportDetail20170
    {
        public string Eid { get; set; }
        public string ReportYear { get; set; }
        public string RegNo { get; set; }
        public string CreditNo { get; set; }
        public string ExId { get; set; }
        public string Name { get; set; }
        public string ReportName { get; set; }
        public string ReportDate { get; set; }
        public string NetAmount { get; set; }
        public string DebitAmount { get; set; }
        public string PracPersonNum { get; set; }
        public string Telephone { get; set; }
        public string Email { get; set; }
        public string SaleIncome { get; set; }
        public string ProfitTotal { get; set; }
        public string ProfitReta { get; set; }
        public string TaxTotal { get; set; }
        public string TotalEquity { get; set; }
        public string Address { get; set; }
        public string FareScope { get; set; }
        public string OperName { get; set; }
        public string ZipCode { get; set; }
        public string ServFareIncome { get; set; }
        public string RegCapi { get; set; }
        public string IndivFormMode { get; set; }
        public string ColleguesNum { get; set; }
        public string IfWebsite { get; set; }
        public string IfInvest { get; set; }
        public string IfExternalGuarantee { get; set; }
        public string IfEquity { get; set; }
        public string CreateDate { get; set; }
        public string Status { get; set; }
        public string Fax { get; set; }
        public string JsonData { get; set; }
        public string GuaranteeItems { get; set; }
        public string InvestItems { get; set; }
        public string Websites { get; set; }
        public string ExternalGuarantees { get; set; }
        public string RegistCapiInfo { get; set; }
        public string ChangeRecords { get; set; }
        public string Branch { get; set; }
        public string StockChanges { get; set; }
        public string InvestSituation { get; set; }
        public string MatterRecord { get; set; }
        public string UpdateRecords { get; set; }
        public string Partners { get; set; }
        public string SocialSecurity { get; set; }
        public DateTime? RowUpdateTime { get; set; }
        public string TelTags { get; set; }
        public string EmailTags { get; set; }
        public string FemaleColleguesNum { get; set; }
        public string EnterpriseHoldingSituation { get; set; }
    }
}
